...
def _get_headnode_dict(fixer_list):
    ...
    for node_type in chain(
        pygram.python_grammar.symbol2number.values(),
...
class RefactoringTool:
    ...
    def get_fixers(self):
        ...
        for fix_mod_path in self.fixers:
            ...
            class_name = self.CLASS_PREFIX + "".join(
            ...
            try:
                ...
                raise FixerError(
                    f"Can't find {fix_name}.{class_name}"
                ) from None
    ...
    def refactor_string(self, data, name):
        ...
        try:
            ...
            self.log_error(
                "Can't parse %s: %s: %s", name, err.__class__.__name__, err
            )
            return
        finally:
        ...
        for line in input.splitlines(keepends=True):
            ...
            if line.lstrip().startswith(self.PS1):
                if block is not None:
                    result.extend(
                        self.refactor_doctest(
                            block, block_lineno, indent, filename
                        )
                    )
                block_lineno = lineno
                block = [line]
...
class MultiprocessRefactoringTool(RefactoringTool):
    ...
    def refactor(
        ...
        processes = [
            multiprocessing.Process(target=self._child)
